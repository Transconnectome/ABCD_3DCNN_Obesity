...
def freeze_conv(net): 
    for k, v in net.module.named_parameters():
        if k.find('conv_') != -1:
        #if k.find('features') or k.find('conv_') != -1:
            """
    ...
    return net 
""""""
def unfreeze_conv(net, global_steps, args): 
    """
    ...
    """
    interval = 90
    flag = [(interval*i)+args.conv_unfreeze_iter for i in range(4)] 
    if global_steps >= flag[0] and global_steps < flag[1]: 
        freeze_layers = ['denseblock%s' % (i+1) for i in range(3)]
        if global_steps == flag[0]:
            print("The 4th dense block is unfrozen")
    elif global_steps >= flag[1] and global_steps < flag[2]: 
        freeze_layers = ['denseblock%s' % (i+1) for i in range(2)]
        if global_steps == flag[1]:
            print("The 3, 4th dense block is unfrozen")
    elif global_steps >= flag[2] and global_steps < flag[3]: 
        freeze_layers = ['denseblock%s' % (i+1) for i in range(1)]   
        if global_steps == flag[2]:
            print("The 2, 3, 4th dense block is unfrozen") 
    else: 
        freeze_layers = []    
        if global_steps == flag[3]:
            print("The all dense block is unfrozen") 
    
    for k, v in net.module.named_parameters():
        if k.find('conv_features') != -1:
            if freeze_layers:
